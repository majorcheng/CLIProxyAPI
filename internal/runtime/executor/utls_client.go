...
// 仅对 Anthropic 官方域名启用 uTLS，避免把伪装 TLS 指纹误伤自定义兼容端点。
var anthropicHosts = map[string]struct{}{
	"api.anthropic.com": {},
}

// utlsRoundTripper 使用 Chrome 指纹建立 HTTP/2 连接，以贴近真实 Claude Code 的 TLS 行为。
...
// newUtlsHTTPClient 为 Claude 主请求链构造 HTTP client：
// - 官方 Anthropic 域名走 uTLS Chrome 指纹
// - 其余域名或非 HTTPS 请求回退到普通 transport
// - 代理优先级保持 auth.ProxyURL > cfg.ProxyURL

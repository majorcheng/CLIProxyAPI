...
func (e *GeminiCLIExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	basePayload = applyPayloadConfigWithRoot(e.cfg, baseModel, "gemini", "request", basePayload, originalTranslated, requestedModel, payloadRequestPath(opts))
	...
	for idx, attemptModel := range models {
		...
		go func(resp *http.Response, reqBody []byte, attemptModel string) {
				...
				for scanner.Scan() {
					...
					if bytes.HasPrefix(line, dataTag) {
						...
						for i := range segments {
							if !sendStreamChunk(ctx, out, cliproxyexecutor.StreamChunk{Payload: []byte(segments[i])}) {
								return
							}
						}
					}
			...
			if errRead != nil {
				recordAPIResponseError(ctx, e.cfg, errRead)
				reporter.publishFailure(ctx)
				if !sendStreamChunk(ctx, out, cliproxyexecutor.StreamChunk{Err: errRead}) {
					return
				}
				return
			}

...
func TestNextRefreshCheckAt_DisabledStillSchedulesRefresh(t *testing.T) {
...
func TestNextRefreshCheckAt_PendingDeleteUnschedule(t *testing.T) {
	now := time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC)
	expiry := now.Add(time.Hour)
	lead := 10 * time.Minute
	setRefreshLeadFactory(t, "pending-delete", func() *time.Duration {
		d := lead
		return &d
	})
...
func TestNextRefreshCheckAt_CodexUsesLastRefreshFallback(t *testing.T) {
	now := time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC)
	lastRefresh := now.Add(-24 * time.Hour)
	auth := &Auth{
		ID:              "codex-last-refresh",
		Provider:        "codex",
	...
	want := lastRefresh.Add(codexLastRefreshStaleWindow)
	if !got.Equal(want) {
		t.Fatalf("nextRefreshCheckAt() = %s, want %s", got, want)
	}
}

func TestManagerStartAutoRefresh_UsesConfiguredWorkerPool(t *testing.T) {
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{AuthAutoRefreshWorkers: 7})
	...
	manager.mu.RUnlock()
	if loop == nil {
		t.Fatal("StartAutoRefresh should initialize refresh loop")
	}
	if loop.concurrency != 7 {
		t.Fatalf("refresh loop worker count = %d, want 7", loop.concurrency)
	}
}

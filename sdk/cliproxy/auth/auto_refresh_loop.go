...
func (l *authAutoRefreshLoop) rebuild(now time.Time) {
	...
	for _, entry := range entries {
		item := &refreshHeapItem{id: entry.id, next: entry.next}
		heap.Push(&l.queue, item)
		l.index[entry.id] = item
	}
	l.mu.Unlock()
}
...
func nextRefreshCheckAt(now time.Time, auth *Auth, interval time.Duration) (time.Time, bool) {
	...
	// Codex 需要和本仓已有的 shouldRefresh 语义保持一致：
	// 优先 access_token exp，其次 metadata expired，最后 last_refresh stale fallback。
	if provider == "codex" {
		return nextCodexRefreshCheckAt(now, auth, lastRefresh, expiry, hasExpiry)
	}
...
func nextCodexRefreshCheckAt(now time.Time, auth *Auth, lastRefresh, expiry time.Time, hasExpiry bool) (time.Time, bool) {
	...
	if tokenExpiry, ok := authCodexAccessTokenExpiry(auth); ok && !tokenExpiry.IsZero() {
		dueAt := tokenExpiry.Add(-codexProactiveRefreshWindow)
		if !dueAt.After(now) {
			return now, true
		}
		return dueAt, true
	...
	if hasExpiry && !expiry.IsZero() {
		dueAt := expiry.Add(-codexProactiveRefreshWindow)
		if !dueAt.After(now) {
			return now, true
		}
		return dueAt, true
	...
	if !lastRefresh.IsZero() {
		dueAt := lastRefresh.Add(codexLastRefreshStaleWindow)
		if !dueAt.After(now) {
			return now, true
		}
		return dueAt, true

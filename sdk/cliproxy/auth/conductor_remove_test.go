...
// TestManager_Remove_DeletesRuntimeAuth 验证 Remove 会从 manager 运行态删除 auth。
...
// TestManager_Update_MissingAuthIsNoOp 验证已删除 auth 的迟到 Update 不会重新插入。
...
// TestManager_Remove_UnschedulesAutoRefresh 验证 Remove 同步取消自动刷新调度。

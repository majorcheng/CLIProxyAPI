...
func TestCleanGeminiCLIRequestSchemasFlattensFunctionDeclarationTypeArray(t *testing.T) {
	...
	resultType := gjson.GetBytes(out, "request.generationConfig.responseSchema.properties.result.type")
	if resultType.String() != "string" {
		t.Fatalf("response schema result.type = %s, want string; body=%s", resultType.Raw, string(out))
	}

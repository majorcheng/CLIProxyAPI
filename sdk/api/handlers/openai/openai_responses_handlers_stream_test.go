...
func TestForwardResponsesStreamRepairsMixedDoneItemsInArrivalOrder(t *testing.T) {
	h, recorder, c, flusher := newResponsesStreamTestHandler(t)

	data := make(chan []byte, 3)
	errs := make(chan *interfaces.ErrorMessage)
	data <- []byte(`data: {"type":"response.output_item.done","item":{"type":"reasoning","id":"rs-1","summary":[]}}`)
	data <- []byte(`data: {"type":"response.output_item.done","output_index":0,"item":{"type":"message","id":"msg-1","role":"assistant","content":[{"type":"output_text","text":"done"}]}}`)
	data <- []byte(`data: {"type":"response.completed","response":{"id":"resp-1","output":[]}}`)
	close(data)
	close(errs)
	...
	if got := gjson.Get(payload, "response.output.0.id").String(); got != "rs-1" {
		t.Fatalf("expected unindexed reasoning to stay first, got %q in %s", got, payload)
	}
	if got := gjson.Get(payload, "response.output.1.id").String(); got != "msg-1" {
		t.Fatalf("expected indexed message to stay second, got %q in %s", got, payload)
	}
}
...
func TestForwardResponsesStreamPreservesBareDoneFrame(t *testing.T) {
	h, recorder, c, flusher := newResponsesStreamTestHandler(t)
	...
	want := "data: [DONE]"
	if got != want {
		t.Fatalf("unexpected bare done frame.\nGot:  %q\nWant: %q", got, want)
	}
}

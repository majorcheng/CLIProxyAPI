...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/internal/audit"
...
const (
	...
	// Codex 按官方思路优先看 AT 自身 exp；本地策略只在到期前 48 小时内做主动 refresh。
	codexProactiveRefreshWindow = 48 * time.Hour
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, maxInvalidRequestRetries int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, execReq, opts, routeModel, models, pooled, release)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				return nil, errCtx
			}
...
// sanitizeDownstreamWebsocketFallbackRequest 在 WebSocket 下游回退到 HTTP auth 时删除私有 generate 字段。
...
func (m *Manager) shouldRefreshCodexFromTokenJSON(a *Auth, now, lastRefresh, expiry time.Time, hasExpiry bool) bool {
	...
	// 1. 优先解析 access token 自身的 exp，并只在到期前 48 小时内主动 refresh；

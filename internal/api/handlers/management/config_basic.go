...
func (h *Handler) PutConfigYAML(c *gin.Context) {
	...
	h.cfg = newCfg
	if h.configApplied != nil {
		h.configApplied(newCfg)
	}
...
func (h *Handler) GetUsageStatisticsPersistIntervalSeconds(c *gin.Context) {
	...
	c.JSON(200, gin.H{"usage-statistics-persist-interval-seconds": cfg.UsageStatisticsPersistIntervalSeconds})
}
func (h *Handler) PutUsageStatisticsPersistIntervalSeconds(c *gin.Context) {
	...
	h.mutateConfig(c, func(cfg *config.Config) {
		cfg.UsageStatisticsPersistIntervalSeconds = value
	})
}
...
func (h *Handler) GetUsageStatisticsRetentionDays(c *gin.Context) {
	...
	c.JSON(200, gin.H{"usage-statistics-retention-days": cfg.UsageStatisticsRetentionDays})
}
func (h *Handler) PutUsageStatisticsRetentionDays(c *gin.Context) {
	...
	h.mutateConfig(c, func(cfg *config.Config) {
		cfg.UsageStatisticsRetentionDays = value
	})
}
...
func (h *Handler) GetMaxInvalidRequestRetries(c *gin.Context) {
	...
	c.JSON(200, gin.H{"max-invalid-request-retries": cfg.MaxInvalidRequestRetries})
}
...
func (h *Handler) GetPriorityZeroRoutingStrategy(c *gin.Context) {
	...
	strategy, ok := normalizePriorityZeroRoutingStrategy(cfg.Routing.PriorityZeroStrategy)
	if !ok {
		c.JSON(200, gin.H{"priority-zero-strategy": strings.TrimSpace(cfg.Routing.PriorityZeroStrategy)})
		return
	}
...
func (h *Handler) PutPriorityZeroRoutingStrategy(c *gin.Context) {
	...
	h.mutateConfig(c, func(cfg *config.Config) {
		cfg.Routing.PriorityZeroStrategy = normalized
	})
}

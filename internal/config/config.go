...
type Config struct {
	...
	// AuthAutoRefreshWorkers overrides the core auth auto-refresh worker pool size.
	// 当配置值 <= 0 时，仍使用内置默认并发数。
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	if err = ValidateOpenAICompatNames(cfg.OpenAICompatibility); err != nil {
		return nil, err
	}
...
// 这里的唯一键必须包含 base-url；否则同一 api-key 指向多个 upstream 时，
// management 删除/编辑会在 sanitize 阶段被静默折叠，导致配置语义丢失。

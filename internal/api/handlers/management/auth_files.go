...
func (h *Handler) GetAuthFileModels(c *gin.Context) {
	...
	if manager != nil {
		if auth, ok := manager.GetByID(name); ok && auth != nil {
			authID = auth.ID
		} else if auth, ok := manager.FindByFileName(name); ok && auth != nil {
			authID = auth.ID
		}
	}
...
func (h *Handler) listAuthFilesFromDisk(c *gin.Context) {
	...
	for _, e := range entries {
		...
		if errValidate := coreauth.ValidatePersistedAuthProvider(typeValue); errValidate != nil {
			log.WithError(errValidate).Warnf("management: skip unsupported auth file %s", full)
			continue
		}
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	if err := coreauth.ValidatePersistedAuthProvider(auth.Provider); err != nil {
		log.WithError(err).Warnf("management: skip unsupported auth entry %s", auth.ID)
		return nil
	}
	auth.EnsureIndex()
...
func (h *Handler) deleteAuthFileByName(ctx context.Context, name string) (string, bool, int, error) {
	...
	cfg := h.currentConfigSnapshot()
	if cfg == nil {
		return "", false, http.StatusServiceUnavailable, fmt.Errorf("configuration unavailable")
	}
...
func (h *Handler) buildAuthFromFileData(path string, data []byte) (*coreauth.Auth, error) {
	...
	if cfg := h.currentConfigSnapshot(); isNewAuth && cfg != nil && cfg.CodexInitialRefreshOnLoad {
...
func (h *Handler) RequestIFlowToken(c *gin.Context) {
	...
	go func() {
		...
		waitFile := filepath.Join(cfg.AuthDir, fmt.Sprintf(".oauth-iflow-%s.oauth", state))
		deadline := time.Now().Add(5 * time.Minute)

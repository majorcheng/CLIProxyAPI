...
func TestManager_Execute_DisableCooling_RetriesAfter429RetryAfter(t *testing.T) {
	...
	executor := &authFallbackExecutor{
		...
		executeErrors: map[string]error{
			"auth-429-retryafter-exec": &codexStressError{
...
const requestScopedNotFoundMessage = "Item with id resp_123 not found; items are not persisted when `store` is set to false"
